...
function addDailySheet(workbook: Workbook, input: StaticReportWorkbookInput, logoImageId?: number) {
  ...
  const columns: TableColumn[] = [
    { header: 'Fecha', width: 17, numberFormat: 'dd mmm yyyy' },
    { header: 'Tipo de registro', width: 22 },
    { header: 'Cupos requeridos', width: 17, numberFormat: '#,##0', alignment: 'right' },
    ...
    { header: 'Hombres', width: 13, numberFormat: '#,##0', alignment: 'right' },
    { header: 'Mujeres', width: 13, numberFormat: '#,##0', alignment: 'right' },
  ...
  const totalMaleAttendance = days.reduce((total, day) => total + (day.maleAttendance || 0), 0);
  const totalFemaleAttendance = days.reduce((total, day) => total + (day.femaleAttendance || 0), 0);
  const hasCompleteGenderBreakdown = days.some((day) => day.totalAttendance != null)
    && days.every((day) => day.totalAttendance == null || (day.maleAttendance != null && day.femaleAttendance != null));
  ...
  writeTable(sheet, columns, days.map(day => [
    toExcelDate(day.date), day.hasScheduledShifts ? 'Cobertura y asistencia' : 'Solo asistencia',
    day.hasScheduledShifts ? day.required : null,
    day.hasScheduledShifts ? day.assigned : null,
    day.hasScheduledShifts ? day.covered : null,
    day.hasScheduledShifts ? day.checkedIn : null,
    day.maleAttendance, day.femaleAttendance, day.totalAttendance,
    day.hasScheduledShifts ? day.missing : null,
    day.hasScheduledShifts ? day.coverageRate / 100 : null,
    ...['T1', 'T2', 'T3', 'T4'].map(shift => day.byShift[shift] ? `${day.byShift[shift].assigned}/${day.byShift[shift].required}` : '—'),
  ]), {
    totalRow: [
      'TOTAL', null, totalRequired, totalAssigned, totalCovered, totalCheckedIn,
      hasCompleteGenderBreakdown ? totalMaleAttendance : null,
      hasCompleteGenderBreakdown ? totalFemaleAttendance : null,
      hasRecordedAttendance ? totalAttendance : null, totalMissing,
