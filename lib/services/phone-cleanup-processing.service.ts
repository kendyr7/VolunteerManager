...
import { VolunteerAuditWriter, AuditActor } from './volunteer-audit-writer';
import { normalizePhoneE164, getLocal8Digits } from '@/lib/whatsapp';
import { PersonCentricDecision, PersonCentricPhoneStatus, PersonCentricStatus, PersonCentricProcessingStatus } from './phone-cleanup-review.service';
    ...
    if (decision === 'PHONE_DOES_NOT_BELONG') {
      if (item.phone_status === 'MISSING_INFORMATION' || !item.corrected_phone) {
        return {
          ...
          code: 'REQUIRES_INFORMATION',
          success: false,
          message: 'Esta persona requiere información. No se proporciona un teléfono nuevo.',
        };
      }
    }
    ...
    if (decision === 'SHARED_PHONE') {
      if (!item.shared_phone_owner_id) {
        return {
          ...
          code: 'CONFLICT',
          success: false,
          message: 'Conflicto: No se especificó el ID del titular para el teléfono compartido.',
        };
      }
      if (item.shared_phone_owner_id === volunteerId) {
        return {
          ...
          code: 'CONFLICT',
          success: false,
          message: 'Conflicto: La persona no puede ser el titular de sí misma en uso compartido.',
        };
      }
      ...
      if (!ownerVol) {
        return {
          ...
          code: 'CONFLICT',
          success: false,
          message: 'Conflicto: El titular asignado no existe en public.volunteers.',
        };
      }
    }
    ...
    if (decision === 'ARCHIVE_DUPLICATE') {
      if (!item.duplicate_primary_volunteer_id) {
        return {
          ...
          code: 'CONFLICT',
          success: false,
          message: 'Conflicto: No se especificó el voluntario principal que conservará el registro.',
        };
      }
      if (item.duplicate_primary_volunteer_id === volunteerId) {
        return {
          ...
          code: 'CONFLICT',
          success: false,
          message: 'Conflicto: El voluntario principal no puede ser la misma persona a archivar.',
        };
      }
    }
