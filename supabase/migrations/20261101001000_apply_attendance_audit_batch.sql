...
begin
  ...
  -- Lock and validate every source row before changing anything.
  ...
  -- Remove accidental scans first so merge/update intervals can expand safely.
  for v_op in
  ...
  -- Do not allow the corrected intervals to overlap another surviving session.
  for v_op in
    select value from pg_catalog.jsonb_array_elements(p_operations)
    where value->>'type' = 'update'
  loop
  ...
  for v_resolution in select value from pg_catalog.jsonb_array_elements(p_resolutions) loop
    insert into public.attendance_review_resolutions (
      session_id, volunteer_id, day_key, decision, explanation, correction,
