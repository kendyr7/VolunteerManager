...
`Detalle del panel` usa `FILTER` y se actualiza automáticamente cuando cambia un desplegable. `Catálogos` alimenta las listas y conserva los filtros originales; `Cálculos` contiene las máscaras que actualizan métricas y detalle. Ambas hojas están ocultas porque son soporte técnico necesario.
