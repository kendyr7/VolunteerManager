...
// APPLY ALL FIXES AND TYPOGRAPHY UPDATES HERE TO PREVENT RE-RUNNING REPLACE TOOLS
...
commCardHtml = commCardHtml.replace(/className="px-8 py-5/g, 'className="px-6 py-3');
commCardHtml = commCardHtml.replace(/className="px-6 py-4/g, 'className="px-6 py-3'); // Fallback if px-6 py-4 is used

// Remove "01", "02"
commCardHtml = commCardHtml.replace(/<span className="text-\[10px\] font-bold text-slate-300 w-4">0\{idx \+ 1\}<\/span>\n\s*/g, '');

// Apply smallcaps text
...
// Remove Faltan Block completely!
