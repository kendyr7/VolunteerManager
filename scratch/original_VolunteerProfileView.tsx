...
export function VolunteerProfileView({
  ...
  const handleSendRescheduleRequest = async () => {
    ...
    if (!requestReason.trim()) {
      setSubmitError("Por favor ingresa la raz├│n o motivo por el cual solicitas el cambio.");
      return;
    }
    ...
    if (res.success) {
      setSubmitSuccess("Γ£à Solicitud enviada exitosamente. El coordinador la revisar├í en breve.");
      await loadRequests();
      setTimeout(() => {
      ...
      setSubmitError(res.error || "Ocurri├│ un error al enviar la solicitud.");
    }
    ...
    <div className="flex flex-col w-full relative">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <span className="text-text-dim text-[11px]">
                {pendingRequests[0].current_shift_key} ({pendingRequests[0].current_day_key}) Γ₧ö {pendingRequests[0].requested_shift_key} ({pendingRequests[0].requested_day_key})
              </span>
            </div>
          </div>
      ...
      <div className="flex items-center mb-6 py-3 border-y border-border -mx-2 sm:mx-0">
        ...
        <div className="flex flex-col items-center flex-1 border-r border-border">
          ...
          <span className="text-drawer-kpi-label text-text-dim mt-1.5 font-inter font-extrabold">D├¡as</span>
        </div>
        <div className="flex flex-col items-center flex-1 border-r border-border">
      ...
      <div className="w-full">
        ...
        {savedNotice && (
          <div className="mb-3 px-3 py-2 bg-emerald-500/15 border border-emerald-500/30 text-emerald-400 text-xs font-bold rounded-xl text-center animate-in fade-in">
            ┬íTurnos actualizados correctamente!
          </div>
        )}
            ...
            <div className="flex items-center justify-between border-b border-border pb-4">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-xs text-text-dim font-medium">Env├¡a tu petici├│n de cambio de fecha o turno.</p>
                </div>
              </div>
              <button
                onClick={() => setIsRescheduleModalOpen(false)}
