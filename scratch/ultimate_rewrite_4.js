...
console.log('Restoring from git HEAD (the original un-responsive state)...');
...
// ==========================================
// STAGE 1: ULTIMATE REWRITE (Responsiveness & Heatmap fixes)
// ==========================================
...
// Note: We bypass the intermediate 'px-6 py-5' steps and go straight to the final desired state
// Remove title header block natively from the string
const headerStartIdx = commCardHtml.indexOf('<div className="px-8 py-7 border-b border-slate-50 flex items-center justify-between">');
if (headerStartIdx !== -1) {
    const linkEndIdx = commCardHtml.indexOf('</Link>', headerStartIdx);
    const headerEndIdx = commCardHtml.indexOf('</div>', linkEndIdx) + 6;
    commCardHtml = commCardHtml.substring(0, headerStartIdx) + commCardHtml.substring(headerEndIdx);
}

// Reduce margin
commCardHtml = commCardHtml.replace(/className="px-8 py-5 flex items-center/g, 'className="px-6 py-1.5 flex items-center');

// Remove numbers
commCardHtml = commCardHtml.replace(/<span className="text-\[10px\] font-bold text-slate-300 w-4">0\{idx \+ 1\}<\/span>\n\s*/g, '');

// Apply typography EXACTLY as requested
commCardHtml = commCardHtml.replace('text-sm font-bold text-slate-700 group-hover:text-slate-900', 'text-xs font-bold text-slate-400 uppercase tracking-wider mt-2 group-hover:text-slate-600');
...
console.log('Complete MASTER rewrite done.');
