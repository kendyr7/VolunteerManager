...
const heatmapLogic = `  const heatmapMatrix = useMemo(() => {
...
`;
content = content.replace(depsLine, depsLine + '\n\n' + heatmapLogic);
...
// 3. Reorganize Layout and Replace Cuellos List with Heatmap
const chartStartMarker = '      {/* Daily Volunteer Distribution Chart */}\n      <motion.div variants={itemVariants}>';
const chartStartIndex = content.indexOf('      {/* Daily Volunteer Distribution Chart */}');
...
chartCardHtml = chartCardHtml.replace('rounded-sm overflow-hidden">', 'rounded-sm overflow-hidden h-full flex flex-col">');
chartCardHtml = chartCardHtml.replace('<CardContent className="p-7">', '<CardContent className="p-7 flex-1 flex flex-col justify-end">');
...
// BULLETPROOF END MATCHING
const heatmapMarkerIndex = content.indexOf('{/* Right: Mapa de Calor Operativo */}');
const commInnerEnd = content.lastIndexOf('        </Card>', heatmapMarkerIndex) + '        </Card>'.length;
...
commCardHtml = commCardHtml.replace(/<div className="flex-1 max-w-md">/g, '<div className="flex-1 w-full">');

// Heatmap Matrix UI
const heatmapUI = `          <Card className="border-none bg-slate-50/50 shadow-inner rounded-sm overflow-hidden h-full flex flex-col border border-slate-200/60">
            ...
            <CardContent className="p-0 flex-1 min-h-[350px]">
              <div className="overflow-x-auto">
                <div className="min-w-[600px] flex">
                  ...
                  <div className="flex-1 grid grid-cols-4 lg:grid-cols-8">
                    {heatmapMatrix.map((dayData, idx) => (
                      <div key={dayData.day} className="flex flex-col border-r border-slate-100/50 last:border-0">
