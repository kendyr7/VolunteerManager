...
const requests = [];
const action = (target, simulation) => new Promise(resolve => requests.push({ target, simulation, resolve }));
...
async function run() {
  ...
  assert(!hasCoverage(84), 'old simulation scope is not displayed during a filter change');
